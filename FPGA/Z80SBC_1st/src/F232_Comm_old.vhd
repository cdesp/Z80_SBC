...
architecture Behavioral of FT232_Comm is
    ...
    signal rx_data_ready    : std_logic := '0';    -- LSR bit 0
...
begin
  ...
  -- ***************************************************************
    ...
    -- Oversampling Counter and Bit Tick Generation (Driven by 16x Clock)
    process(CLK)
    begin
        if rising_edge(CLK) then
...
process(data_length, PARITY_EN, STOP_BIT_LEN)
    -- total_bits should be a variable or signal defined in the architecture or process scope
    variable v_parity_bit : integer := 0;
    variable v_stop_bits  : integer := 0;
begin
    -- 1. Calculate Parity Bit contribution
    if PARITY_EN = '1' then
        v_parity_bit := 1;
    else
        v_parity_bit := 0;
    ...
    -- 2. Calculate Stop Bits contribution
    -- Assuming STOP_BIT_LEN = '1' means 2 bits, and '0' means 1 bit.
    ...
    -- If 'total_bits' is a signal, assign it using '<='
    -- If 'total_bits' is a variable (defined here), assign it using ':='
    total_bits <= data_length + v_parity_bit + v_stop_bits;
    ...
    begin
        if rising_edge(CLK) then
            if RESET_N = '0' then
                ...
                -- Dynamic configuration based on LCR
                data_length <= to_integer(unsigned("01" & WORD_LENGTH) + 5); -- 5-8 bits
                --total_bits := data_length + (1 when PARITY_EN = '1' else 0) + (2 when STOP_BIT_LEN = '1' else 1); 
                ...
                case rx_state is
                    when RX_IDLE =>
                        ...
                        if RXD_sync(2 downto 1) = "10" then
                            ...
                            os_count <= (others => '0'); -- Reset OS counter
                    ...
                    when RX_PARITY =>
                        if bit_tick = '1' then
                            ...
                            -- Odd Parity: calculated_parity MUST be 0 (odd number of 1s in data)
                            -- Even Parity: calculated_parity MUST be 1 (even number of 1s in data)
                            if (EVEN_PARITY = '0' and calculated_parity = '1' and RXD_sync(2) = '0') or  -- Odd Parity Error (Odd data, expected 0, got 0)
                               (EVEN_PARITY = '1' and calculated_parity = '0' and RXD_sync(2) = '1') then -- Even Parity Error (Even data, expected 1, got 1)
                                rx_parity_error <= '1';
                            else
                    ...
                    when RX_STOP =>
                        if bit_tick = '1' then
                            ...
                            if STOP_BIT_LEN = '1' and rx_bit_count = (total_bits - 1) then
                                -- Stay in RX_STOP for the second stop bit
                                null;
                            else
    ...
    -- Main TX Process
    process(CLK)
        variable bits_to_send : integer range 5 to 8;
    ...
    begin
        if rising_edge(CLK) then
            if RESET_N = '0' then
                ...
                bits_to_send := to_integer(unsigned("101" & WORD_LENGTH) - 5); 
                parity_enable_v := PARITY_EN;
                
                -- Calculate parity bit for the TX frame
                ...
                if EVEN_PARITY = '1' then p_bit := NOT p_bit; end if; -- Even Parity: invert for even number of 1s
                
                case tx_state is
                    when TX_IDLE =>
                        tx_busy <= '0';
                        if tx_data_available = '1' then
                            -- Construct the frame: [Stop Bits][Parity Bit][Data Bits][Start Bit]
                            if PARITY_EN = '1' then
                                shift_load := STOP_BIT_LEN & p_bit & tx_buffer & START_BIT; 
                            else
                                -- No Parity
                                shift_load := STOP_BIT_LEN & '1' & tx_buffer & START_BIT; -- Placeholder parity bit '1'
                            end if;
                    ...
                    when TX_DATA =>
                        tx_bit_count <= tx_bit_count + 1;
                        -- Assuming 8-bit data for now, ignoring LCR[1:0]
                    ...
                    when TX_STOP =>
                        tx_bit_count <= tx_bit_count + 1;
                        -- Check for two stop bits (LCR[2] = '1')
                        if STOP_BIT_LEN = '1' and tx_bit_count < 10 then -- Total bits (8 data + 1 start + 2 stop)
                            null; -- Continue to the second stop bit
                        else
                            tx_state <= TX_IDLE;
                        end if;
    ...
    -- ** READ LOGIC (UART -> Z80) **
    -- Note: Reading RBR clears DR and all error flags (OE, PE, FE, BI)
    process(CLK)
    begin
        if rising_edge(CLK) then
            if CS_THIS_PORT = '1' and IO_ADDR(0) = '0' and RD_N = '0' and DLAB = '0' then
                -- Reading RBR clears the Data Ready flag and all error flags
                rx_data_ready <= '0'; 
                rx_overrun_error <= '0'; 
                rx_parity_error <= '0';
                rx_framing_error <= '0';
    ...
    -- This section computes the 8-bit value that should be put on the bus 
    -- when the Z80 is actively reading this port.

    with IO_ADDR(2 downto 0) select read_data <=
    -- RBR/DLL (Address 000)
    -- Check DLAB flag to select between Receiver Buffer Register (RBR) or 
    ...
    -- IER/DLM (Address 001)
    -- Check DLAB flag to select between Interrupt Enable Register (IER) or
    -- Divisor Latch High (DLM) register.
    (
    ...
    -- This part remains correct and handles the tristate output.
    IO_DATA_OUT <= 
