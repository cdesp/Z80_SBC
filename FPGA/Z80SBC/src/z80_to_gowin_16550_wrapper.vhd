...
architecture Behavioral of z80_to_gowin_16550_wrapper is
    ...
    signal reg_lsr_bit5     : std_logic := '0';
    signal reg_lsr_bit0     : std_logic := '0';
...
begin
    ...
    sCTSn <= ip_rtsn or (not reg_mcr(0));
...
----------------------------------------------------------------------------
    ...
    begin
        if rising_edge(CLK_Z80) then
            if rst_high = '1' then
                ...
                case ADDR_BUS is
                    when "011" => dlab_reg <= DATA_BUS_IN(7);
                    when "100" => reg_mcr  <= DATA_BUS_IN; -- Capturing the MCR
                    when "000" => if dlab_reg = '1' then reg_dll <= DATA_BUS_IN; end if;
                    when "001" => if dlab_reg = '1' then reg_dlm <= DATA_BUS_IN; end if;
                    when others => null;
                end case;
            end if;
        end if;
    ...
    begin
        if rising_edge(CLK_FPGA) then
            if rst_high = '1' then
                wr_sync_reg1   <= '0'; wr_sync_reg2 <= '0'; wr_sync_reg3 <= '0'; wr_sync_reg4 <= '0';
...
if (wr_sync_reg2 = '1' and wr_sync_reg3 = '0') then
    ip_wdata <= DATA_BUS_IN;
    -- Don't pulse EN yet! Just set the data
end if;

if (wr_sync_reg3 = '1' and wr_sync_reg4 = '0') then -- Add a 4th sync register
    ip_tx_en <= '1'; -- Pulse EN after data has had one cycle to settle
    tx_hw_lock <= '1';
end if;
...
begin
    if rising_edge(CLK_FPGA) then
        -- We hold Bit 5 low as long as the TX is locked OR the IP says it's busy
        if (tx_hw_lock = '1') or (ip_txrdyn = '1') then
            reg_lsr_bit5 <= '0'; -- Busy
        else
            reg_lsr_bit5 <= '1'; -- Ready
        end if;
        
        reg_lsr_bit0 <= not ip_rxrdyn;
    end if;
end process;
   ...
   -- ip_rx_en <= '1' when (z80_cs_pulse = '1' and L_RD_N = '0' and Z80_LA_BUS_INT = "000" and dlab_reg = '0') else '0';
    ...
    process(ADDR_BUS, dlab_reg, reg_dll, reg_dlm, ip_rdata, ip_txrdyn, ip_rxrdyn, tx_hw_lock,reg_lsr_bit5,reg_lsr_bit0)
    ...
    begin
       -- Synthesize a vintage 16550 Line Status Register (LSR) byte
        ...
        fake_lsr(5) := reg_lsr_bit5;--(not ip_txrdyn) and (not tx_hw_lock); 
        fake_lsr(6) := not ip_txrdyn; 
        fake_lsr(0) := reg_lsr_bit0;--not ip_rxrdyn;
