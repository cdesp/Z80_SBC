...
USE IEEE.STD_LOGIC_ARITH.ALL; -- Χρησιμοποιείται για conv_integer (compatibility)
...
ENTITY MMU IS
        PORT (
                ...
                -- I/O signal for Page Mapping Registers (OUT (c), a)
                nSET_RO      :   IN  STD_LOGIC;
                -- NEW: I/O signal for Read-Only (OUT (protInt), PgNo)
                nSET_RW      :   IN  STD_LOGIC;
                -- NEW: I/O signal for Read/Write (OUT (protInt+1), PgNo)
                nWR_CPU      :   IN  STD_LOGIC;
                -- Z80 Write Strobe (Active Low)
                nRESET      :   IN  STD_LOGIC;
                DATA        :   IN  STD_LOGIC_VECTOR(7 DOWNTO 0);
                -- Data bus from CPU (PgNo for protection or mapping)
                EA          :   OUT STD_LOGIC_VECTOR(20 DOWNTO 13);
                -- Extended Address bus to mem chips (Page Number)
                nWR_OUT      :   OUT STD_LOGIC;
                -- Protected Write Strobe to physical memory
...
ARCHITECTURE behavioral OF MMU IS
        ...
        -- Μετατροπή σε integer για ευρετηρίαση.
...
BEGIN
        ...
        Prot_Write_Page_Int <= conv_integer(unsigned(DATA));
        ...
        nSetBANK0 <= '0' WHEN BANKNUM = "000" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';
        nSetBANK1 <= '0' WHEN BANKNUM = "001" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';
        nSetBANK2 <= '0' WHEN BANKNUM = "010" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';
        nSetBANK3 <= '0' WHEN BANKNUM = "011" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';
        nSetBANK4 <= '0' WHEN BANKNUM = "100" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';
        nSetBANK5 <= '0' WHEN BANKNUM = "101" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';
        nSetBANK6 <= '0' WHEN BANKNUM = "110" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';
        nSetBANK7 <= '0' WHEN BANKNUM = "111" AND nINTMMU = '0' AND nWR_CPU = '0' ELSE
                '1';

        -- Instantiate 8-bit bank registers. (INIT values preserved)
        Bankreg0 : regn
        GENERIC MAP(N => 8, INIT => x"20")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK0, Q => Bank0);

        Bankreg1 : regn
        GENERIC MAP(N => 8, INIT => x"21")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK1, Q => Bank1);

        Bankreg2 : regn
        GENERIC MAP(N => 8, INIT => x"22")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK2, Q => Bank2);

        Bankreg3 : regn
        GENERIC MAP(N => 8, INIT => x"23")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK3, Q => Bank3);

        Bankreg4 : regn
        GENERIC MAP(N => 8, INIT => x"24")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK4, Q => Bank4);

        Bankreg5 : regn
        GENERIC MAP(N => 8, INIT => x"25")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK5, Q => Bank5);

        Bankreg6 : regn
        GENERIC MAP(N => 8, INIT => x"26")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK6, Q => Bank6);

        Bankreg7 : regn
        GENERIC MAP(N => 8, INIT => x"27")
        PORT MAP(D => DATA, Resetn => nRST, Clock => nSetBANK7, Q => Bank7);
        ...
        EXTADDR <= Bank0 WHEN BANKNUM = "000"
                ELSE
                Bank1 WHEN BANKNUM = "001"
                ELSE
                Bank2 WHEN BANKNUM = "010"
                ELSE
                Bank3 WHEN BANKNUM = "011"
                ELSE
                Bank4 WHEN BANKNUM = "100"
                ELSE
                Bank5 WHEN BANKNUM = "101"
                ELSE
                Bank6 WHEN BANKNUM = "110"
                ELSE
                Bank7 WHEN BANKNUM = "111"
                ELSE
                Bank0;

        -- Present extended address onto EA when memory request active (nMREQ='0')
        EA <= EXTADDR WHEN nMREQ = '0' ELSE
                (OTHERS => '0');

        -- Μετατροπή του τρέχοντος χαρτογραφημένου Page Number (από το EA[20:13]) σε integer
        Current_Mapped_Page_Int <= conv_integer(unsigned(EA(20 DOWNTO 13)));
        ...
        nWR_OUT <= '1' WHEN (nMREQ = '0' AND Write_Protected_Status = '1') -- Block write if Protected (RO)
                ELSE
                nWR_CPU; -- Διαφορετικά, περνάει το nWR_CPU
        -- -------------------------------------------------------------
        -- 5. Chip Select Logic (nCEx) - Unchanged
        -- -------------------------------------------------------------

        -- nCE0 : SRAM 1MB -> blocks 0..31  ("000000" .. "011111")
        nCE0 <= '0' WHEN EXTADDR(20 DOWNTO 15) >= "000000" AND EXTADDR(20 DOWNTO 15) <= "011111" AND nMREQ = '0' ELSE
                '1';

        -- nCE1 : Flash 512KB -> blocks 32..47 ("100000" .. "101111")
        nCE1 <= '0' WHEN EXTADDR(20 DOWNTO 15) >= "100000" AND EXTADDR(20 DOWNTO 15) <= "101111" AND nMREQ = '0' ELSE
                '1';

        -- nCE2 : Internal dual-port video RAM 64KB -> blocks 48..49 ("110000" .. "110001")
        nCE2 <= '0' WHEN EXTADDR(20 DOWNTO 15) >= "110000" AND EXTADDR(20 DOWNTO 15) <= "110001" AND nMREQ = '0' ELSE
                '1';
